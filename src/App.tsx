...
const getOpponentMoveQuote = (pokeName: string, moveName: string, langCode: string) => {
  ...
  const translations: Record<string, Record<string, string>> = {
    it: {
      ...
      psychic: "La forza della mente supera la gravità!",
      dragon: "La rabbia del drago imperverserà!",
    ...
    es: {
      default: `¡Aquí está el poder de ${pokeName}!`,
      fire: "¡Que la llama eterna te consuma!",
      water: "¡Arrastrado por las corrientes del océano!",
      electric: "¡Relámpago de alta tensión eléctrica!",
      grass: "¡La naturaleza reclama su poder!",
      ice: "¡Siente el frío absoluto!",
      psychic: "¡Mi mentalidad supera cualquier fuerza física!",
      dragon: "¡La ira del dragón se desata!",
      boost: "¡Sintiendo la máxima energía competitiva!",
      protect: "¡Escudo de energía impenetrable!",
      healing: "¡Restauración vital iniciada!"
    },
    fr: {
      default: `Voici le pouvoir de ${pokeName}!`,
      fire: "Que les flammes te consument !",
      water: "Emporté par le courant marin !",
      electric: "Décharge haute tension foudroyante !",
      grass: "La nature reprend ses droits !",
      ice: "Ressens le froid absolu !",
      psychic: "La force de l'esprit transcende la matière !",
      dragon: "La colère du dragon fait rage !",
      boost: "Mon énergie atteint son paroxysme !",
      protect: "Bouclier d'énergie impénétrable !",
      healing: "Régénération d'énergie entamée !"
    },
    de: {
      default: `Sieh die wahre Macht von ${pokeName}!`,
      fire: "Lass die Flammen dich verzehren!",
      water: "Weggespült von der Flut!",
      electric: "Hochspannungsschock aktiv!",
      grass: "Die Kraft der Natur holt sich den Sieg!",
      ice: "Erfriere im absoluten Nullpunkt!",
      psychic: "Die Kraft des Geistes überwindet alles!",
...
const tcgCache: Record<string, string> = {};

const PokemonTcgCard = memo(({ displayId, pokemonName, className }: { displayId: string; pokemonName: string; className?: string }) => {
    const [imgSrc, setImgSrc] = useState<string | null>(tcgCache[displayId] || null);
  const [error, setError] = useState(false);

  useEffect(() => {
    if (tcgCache[displayId]) {
      setImgSrc(tcgCache[displayId]);
      return;
    }
    ...
    const timer = setTimeout(() => {
      fetch(`https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:${displayId}&orderBy=-set.releaseDate&pageSize=1`)
        .then(r => r.json())
        .then(d => {
          if (isMounted) {
            if (d.data && d.data.length > 0) {
              const url = d.data[0].images.small;
              tcgCache[displayId] = url;
              setImgSrc(url);
            } else {
              setError(true);
            }
  ...
  if (error) {
    return (
      <div className={cn(className, "border border-red-500/50 bg-red-950 flex justify-center items-center rounded text-[8px] text-red-500 font-mono text-center p-2")}>
        Card Not Found
      </div>
    );
  }
  ...
  if (!imgSrc) {
    return (
      <div className={cn(className, "animate-pulse bg-slate-800/50 rounded border border-slate-700 w-full h-full")}></div>
    );
  }

  return <img src={imgSrc} alt={pokemonName} className={cn(className, "object-contain rounded shadow-[0_0_15px_rgba(0,0,0,0.5)]")} referrerPolicy="no-referrer" draggable={false} onError={(e) => (e.currentTarget.style.display = 'none')} />;
});
...
const OFFLINE_QUIZ_FALLBACK = {
  ...
  questions: [
    ...
    },
    {
      ...
      answerIndex: 1,
      explanation: "Arceus is the divine prime creator who hatched from the cosmic egg in a void of nothingness, whereas Mew acts as the biological stem-ancestor containing the DNA of all non-deity Pokémon."
    },
    {
...
export default function App() {
  ...
  const runDiagnosticsCheck = useCallback(() => {
    ...
    if (wsRef.current && wsStatus === 'connected') {
      ...
      const logSteps = [
        { text: "[DATABASE] Loading Pokédex data... Loaded 1025 Pokémon.", delay: 400, progress: 20 },
        ...
        { text: "[SUCCESS] Setup complete! Welcome to Pokéthology.", delay: 2600, progress: 100 }
      ];
  ...
  const handleBattleAnalysis = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'model', text: `⚠️ **SYSTEM ERROR**: ${err.message}` }]);
    } finally {
        ...
        {isRebooting && (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2">
              <h2 className="text-red-400 font-hud text-xl font-bold tracking-[0.3em] uppercase">Pokédex Syncing</h2>
                              ...
                              <div className="relative w-44 h-44 sm:w-56 sm:h-56 mb-4 group shrink-0">
                                ...
                                <div className="absolute -right-4 top-0 flex flex-col gap-2 z-20">
                                  ...
                                  {['pyroar', 'unfezant', 'frillish', 'jellicent', 'hippowdon', 'hippopotas', 'meowstic', 'indeedee', 'oinkologne', 'basculegion'].includes(pokemon?.name?.split('-')[0].toLowerCase()) && (
                                    ...
                                    >
                                      <HUDCorners />
                                      <span className={cn("text-xs font-black w-4 h-4 leading-none text-center flex items-center justify-center", isFemale ? "text-pink-400" : "text-blue-400")}>{isFemale ? '♀' : '♂'}</span>
                                    </button>
                                  )}
                                </div>
                              </div>
                                          ...
                                          <div className="flex flex-col">
                                            <div className="flex items-center gap-2">
                                              <span className={cn("text-[9px] font-black tracking-wider font-hud uppercase tracking-[0.15em] leading-tight", quotaLimitReached ? "text-red-500" : "text-cyan-300")}>
                                                {quotaLimitReached ? "AI OFFLINE" : "Pokéthology AI"}
                                              </span>
                                  ...
                                  >
                                    ...
                                    <div className="w-full flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-start max-w-full pb-2 sm:pb-3">
                                      ...
                                      <div className="absolute inset-0 z-0 rounded-xl sm:rounded-2xl overflow-hidden pointer-events-none">
                                        {/* Dynamic CSS split-gradient representing the clashing Pokémon types */}
                                        <div 
                                        ...
                                        <div className="absolute bottom-1 left-2 xs:bottom-2 xs:left-4 sm:bottom-6 sm:left-12 md:bottom-8 md:left-16 lg:bottom-10 lg:left-24 pointer-events-auto z-10">
                                          ...
                                          >
                                            <div className="relative h-32 w-32 xs:h-36 xs:w-36 sm:h-48 sm:w-48 md:h-56 md:w-56 lg:h-64 lg:w-64 xl:h-72 xl:w-72 flex items-center justify-center max-h-[40vh]">
                                              ...
                                              <div className="hidden xs:block absolute -bottom-6 text-[7px] text-cyan-400/50 font-hud uppercase tracking-wider whitespace-nowrap">tap Pokémon to play cry and rotate</div>
                                            </div>
                                              ...
                                              <div className="bg-slate-950/90 border border-slate-800/80 rounded-xl p-3 space-y-3 ">
                                                {!battleOpponent ? (
                                                  <div className="text-center py-6 text-slate-500 text-[10px] uppercase font-mono tracking-wider animate-pulse">
                                                    ⚠️ Select a battle opponent to enable tactical combat options comparison.
                                                  </div>
                                                ) : selectedMoves.length === 0 ? (
                                                  <div className="text-center py-6 text-slate-500 text-[10px] uppercase font-mono tracking-wider animate-pulse">
                                                    ⚔️ Select at least one active move to compare combat options against {battleOpponent?.name?.toUpperCase()}.
                                                  </div>
                                                ) : (
                                                  <div className="space-y-3">
                          ...
                          >
                            ...
                            {displayLimit < sortedAndFilteredList.length && (
                              <div className="flex justify-center mt-4 mb-8">
                                ...
                                >
                                  <HUDCorners />
                                  LOAD MORE POKÉMON ({sortedAndFilteredList.length - displayLimit} REMAINING)
                                </button>
                              </div>
                            )}
                          </div>
            ...
            >
              ...
              >
                ...
                <div className="flex flex-col gap-3 bg-slate-950/40 p-3.5 sm:p-4 rounded-xl border border-slate-800/60 shadow-inner w-full text-center">
                  ...
                  {/* API Quota Usage Progress Bar */}
                  <ApiQuotaMonitor 
                    quotaLimitReached={quotaLimitReached} 
                    onQuotaLimitChange={setQuotaLimitReached} 
                  />
                </div>
            ...
            >
              ...
              >
                ...
                <p className="text-[11px] sm:text-xs text-slate-400 leading-relaxed max-w-md">
                  Your tactical foresight, type matchmaking, and simulation results have been logged to the central encyclopedia core. You are now designated a <span className="text-amber-400 font-bold font-hud uppercase">Certified Pokéthology Combatant</span> for today.
                </p>
